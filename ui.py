...
from smart_home_page_new import VedaHomePage, _DeviceTile
...
LOGO_FILE  = BASE_DIR / "assets" / "Veda_Lite_Logo.png"
LOGO_ICO   = BASE_DIR / "assets" / "Veda_Lite_Logo.ico"
    ...
    def set_ai_state(self, state: str) -> None:
        if self._web_view:
            try:
                ...
                self._web_view.page().runJavaScript(f"if(window.setVedaState) window.setVedaState('{st}');")
            except Exception:
                pass
...
class RemoteKeyOverlay(QWidget):
    ...
    def __init__(self, url: str, key: str, auto: str, manual: str, parent=None):
        ...
        subtitle = QLabel("Scan the QR code with your phone to remotely control Veda Echo.")
    ...
    def mark_connected(self) -> None:
        ...
        self._timer_lbl.setText("Phone connected. Veda Echo remote is ready.")
...
class _OrbCenterMuteButton(QPushButton):
    """
    Circular futuristic glassmorphism button located precisely in the center of the AI Orb.
    Toggles microphone mute / unmute state with smooth visual response.
    """
    def __init__(self, parent=None):
        super().__init__(parent)
        ...
        self.setToolTip("Click to Mute / Unmute Microphone (F4)")
    ...
    def paintEvent(self, event):
        ...
        # Background circular glass fill
        bg_grad = QRadialGradient(cx, cy, r)
        if self.muted:
            bg_grad.setColorAt(0.0, QColor(45, 12, 12, 235))
            bg_grad.setColorAt(0.7, QColor(25, 8, 8, 245))
            bg_grad.setColorAt(1.0, QColor(15, 5, 5, 255))
            ring_col = QColor(255, 68, 68, 220 if self.hovered else 170)
        elif self.speaking:
            bg_grad.setColorAt(0.0, QColor(48, 38, 10, 240))
            bg_grad.setColorAt(0.7, QColor(25, 18, 5, 250))
            bg_grad.setColorAt(1.0, QColor(12, 10, 5, 255))
            ring_col = QColor(255, 195, 0, 255 if self.hovered else 220)
        else:
            bg_grad.setColorAt(0.0, QColor(22, 26, 35, 235))
            bg_grad.setColorAt(0.7, QColor(12, 14, 20, 245))
            bg_grad.setColorAt(1.0, QColor(8, 9, 14, 255))
            ring_col = QColor(255, 179, 0, 220 if self.hovered else 150)

        p.setBrush(QBrush(bg_grad))
        p.setPen(Qt.PenStyle.NoPen)
        p.drawEllipse(QPointF(cx, cy), r, r)

        # Outer ring with subtle halo
        p.setBrush(Qt.BrushStyle.NoBrush)
        pen = QPen(ring_col, 2.2 if self.hovered else 1.8)
        p.setPen(pen)
        p.drawEllipse(QPointF(cx, cy), r, r)

        if self.hovered:
            p.setPen(QPen(QColor(ring_col.red(), ring_col.green(), ring_col.blue(), 60), 4.0))
            p.drawEllipse(QPointF(cx, cy), r + 2, r + 2)

        # Center Microphone Vector Icon
        icon_col = QColor(255, 85, 85) if self.muted else (QColor(255, 255, 255) if not self.speaking else QColor(255, 220, 100))
        p.setPen(QPen(icon_col, 2.0, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap, Qt.PenJoinStyle.RoundJoin))
        p.setBrush(QBrush(icon_col) if not self.muted else Qt.BrushStyle.NoBrush)

        mic_y = cy - 4.0
        # Mic capsule
        p.drawRoundedRect(QRectF(cx - 5.0, mic_y - 12.0, 10.0, 15.0), 5.0, 5.0)

        # Mic cradle / bowl
        p.setBrush(Qt.BrushStyle.NoBrush)
        p.drawArc(QRectF(cx - 8.5, mic_y - 6.0, 17.0, 14.0), 0, -180 * 16)

        # Stem & Base
        p.drawLine(QPointF(cx, mic_y + 8.0), QPointF(cx, mic_y + 13.0))
        p.drawLine(QPointF(cx - 6.0, mic_y + 13.0), QPointF(cx + 6.0, mic_y + 13.0))

        # If muted: draw red strike-through diagonal slash
        if self.muted:
            slash_pen = QPen(QColor(255, 55, 55, 250), 2.5, Qt.PenStyle.SolidLine, Qt.PenCapStyle.RoundCap)
            p.setPen(slash_pen)
            p.drawLine(QPointF(cx - 11.0, mic_y - 12.0), QPointF(cx + 11.0, mic_y + 14.0))

        # Status subtitle below icon
        status_txt = "MUTED" if self.muted else ("SPEAKING" if self.speaking else "UNMUTED")
        p.setFont(QFont("Segoe UI", 7, QFont.Weight.Bold))
        text_col = QColor(255, 90, 90, 230) if self.muted else QColor(255, 179, 0, 220)
        p.setPen(text_col)
        p.drawText(QRectF(0, cy + 17.0, W, 13.0), Qt.AlignmentFlag.AlignCenter, status_txt)


class VedaOrbWidget(QWidget):
    """
    Living AI Orb widget for the Veda AI / Veda Echo Dashboard.
    Paints dynamic animated multi-layer luminous plasma waves, orbit rings,
    and particle energy swirls reacting to AI state, with an interactive
    Mute / Unmute button positioned dead-center inside the orb.
    """
    ...
    def __init__(self, parent=None):
        ...
        for i in range(24):
            ...
            dist = random.uniform(75, 135)
            spd = random.uniform(0.008, 0.022) * (1 if i % 2 == 0 else -1)
            self._particles.append({
                "angle": angle,
                "dist": dist,
                "spd": spd,
        ...
        # Center mute button
        self._btn_mute = _OrbCenterMuteButton(self)
        ...
        self._anim_tmr.start(20)

    def resizeEvent(self, event):
        super().resizeEvent(event)
        # Keep center button precisely positioned in the center
        W, H = self.width(), self.height()
        bw, bh = self._btn_mute.width(), self._btn_mute.height()
        self._btn_mute.move(int((W - bw) / 2), int((H - bh) / 2))

    def set_muted(self, muted: bool):
        self.muted = muted
        self._btn_mute.set_muted(muted)
        self.update()

    def set_state(self, state: str):
        self.state = state
        self.speaking = (state == "SPEAKING")
        self._btn_mute.set_state(state)
        self.update()

    def set_speaking(self, speaking: bool):
        self.speaking = speaking
        self._btn_mute.set_state("SPEAKING" if speaking else self.state)
        self.update()

    def _step(self):
        self._tick += 1
        speed_mult = 2.5 if self.speaking else (1.4 if self.state == "THINKING" else 1.0)
        self._rot_1 = (self._rot_1 + 1.2 * speed_mult) % 360.0
        self._rot_2 = (self._rot_2 - 0.8 * speed_mult) % 360.0
        self._rot_3 = (self._rot_3 + 1.6 * speed_mult) % 360.0

        # Pulse ring expansion
        self._pulse_ring += 1.8 * speed_mult
        if self._pulse_ring > 120.0:
            self._pulse_ring = 0.0

        # Target scale & halo
        if self.speaking:
            self._tgt_scale = 1.0 + 0.06 * math.sin(self._tick * 0.18)
            self._tgt_halo = 130.0 + 35.0 * math.sin(self._tick * 0.22)
        elif self.muted:
            self._tgt_scale = 0.98 + 0.01 * math.sin(self._tick * 0.05)
            self._tgt_halo = 45.0
        elif self.state == "THINKING":
            self._tgt_scale = 1.02 + 0.03 * math.sin(self._tick * 0.12)
            self._tgt_halo = 95.0
        else: # LISTENING / IDLE
            self._tgt_scale = 1.0 + 0.02 * math.sin(self._tick * 0.08)
            self._tgt_halo = 75.0 + 15.0 * math.sin(self._tick * 0.1)

        self._scale += (self._tgt_scale - self._scale) * 0.15
        self._halo += (self._tgt_halo - self._halo) * 0.15

        # Update orbiting particles
        for p in self._particles:
            p["angle"] = (p["angle"] + p["spd"] * speed_mult) % (2 * math.pi)
    ...
    def paintEvent(self, _):
        ...
        W, H = self.width(), self.height()
        cx, cy = W / 2.0, H / 2.0
        # Expanded base radius so orb is immersive and spreads across screen
        base_r = min(W, H) * 0.42 * self._scale

        # Primary palette based on state
        if self.muted:
            core_c1 = QColor(255, 68, 68)
            core_c2 = QColor(140, 15, 15)
            glow_c  = QColor(255, 55, 55, 45)
            ring_c  = QColor(255, 80, 80)
        elif self.speaking:
            core_c1 = QColor(255, 215, 0)
            core_c2 = QColor(255, 140, 0)
            glow_c  = QColor(255, 190, 0, 95)
            ring_c  = QColor(255, 230, 100)
        elif self.state == "THINKING":
            core_c1 = QColor(0, 230, 255)
            core_c2 = QColor(255, 179, 0)
            glow_c  = QColor(0, 200, 255, 80)
            ring_c  = QColor(120, 235, 255)
        else: # LISTENING / IDLE
            core_c1 = QColor(255, 185, 0)
            core_c2 = QColor(210, 120, 0)
            glow_c  = QColor(255, 179, 0, 70)
            ring_c  = QColor(255, 195, 40)

        # 1. Full-screen ambient atmospheric glow spanning to edges
        screen_halo = max(W, H) * 0.65
        outer_grad = QRadialGradient(cx, cy, screen_halo)
        outer_grad.setColorAt(0.0, QColor(glow_c.red(), glow_c.green(), glow_c.blue(), int(glow_c.alpha() * 1.0)))
        outer_grad.setColorAt(0.3, QColor(glow_c.red(), glow_c.green(), glow_c.blue(), int(glow_c.alpha() * 0.45)))
        outer_grad.setColorAt(0.65, QColor(glow_c.red(), glow_c.green(), glow_c.blue(), int(glow_c.alpha() * 0.15)))
        outer_grad.setColorAt(1.0, QColor(0, 0, 0, 0))
        p.setBrush(QBrush(outer_grad))
        p.setPen(Qt.PenStyle.NoPen)
        p.drawEllipse(QPointF(cx, cy), screen_halo, screen_halo)

        # 2. Expanding acoustic pulse rings (especially when speaking or active)
        if not self.muted:
            pulse_rad = base_r + self._pulse_ring * 1.5
            pulse_alpha = max(0, int(110 * (1.0 - self._pulse_ring / 120.0)))
            p.setPen(QPen(QColor(ring_c.red(), ring_c.green(), ring_c.blue(), pulse_alpha), 2.0))
            p.setBrush(Qt.BrushStyle.NoBrush)
            p.drawEllipse(QPointF(cx, cy), pulse_rad, pulse_rad)

            # Secondary harmonic wave
            pulse_rad2 = base_r + ((self._pulse_ring + 60.0) % 120.0) * 1.4
            pulse_alpha2 = max(0, int(60 * (1.0 - ((self._pulse_ring + 60.0) % 120.0) / 120.0)))
            p.setPen(QPen(QColor(ring_c.red(), ring_c.green(), ring_c.blue(), pulse_alpha2), 1.2))
            p.drawEllipse(QPointF(cx, cy), pulse_rad2, pulse_rad2)

        # 3. Rotating Orbital Energy Rings spanning widely
        p.save()
        p.translate(cx, cy)
        p.rotate(self._rot_1)
        pen1 = QPen(QColor(ring_c.red(), ring_c.green(), ring_c.blue(), 130), 1.8)
        pen1.setDashPattern([8, 12, 2, 12])
        p.setPen(pen1)
        r1 = base_r * 1.25
        ...
        p.save()
        p.translate(cx, cy)
        p.rotate(self._rot_2)
        pen2 = QPen(QColor(ring_c.red(), ring_c.green(), ring_c.blue(), 95), 1.4)
        pen2.setDashPattern([24, 24])
        p.setPen(pen2)
        r2 = base_r * 1.12
        ...
        p.save()
        p.translate(cx, cy)
        p.rotate(self._rot_3)
        pen3 = QPen(QColor(ring_c.red(), ring_c.green(), ring_c.blue(), 80), 1.0)
        p.setPen(pen3)
        r3 = base_r * 0.95
        p.drawEllipse(QPointF(0, 0), r3, r3)
        p.restore()

        # 4. Floating swirling particles with multi-depth field
        for pt in self._particles:
            spread = (base_r * 0.9) + (pt["dist"] * 0.5 * self._scale)
            px = cx + math.cos(pt["angle"]) * spread
            py = cy + math.sin(pt["angle"]) * spread
            p_alpha = int(255 * pt["alpha"])
            p.setPen(Qt.PenStyle.NoPen)
            p.setBrush(QBrush(QColor(ring_c.red(), ring_c.green(), ring_c.blue(), p_alpha)))
            p.drawEllipse(QPointF(px, py), pt["size"], pt["size"])

        # 5. Core 3D Volumetric Sphere (underlying the center button)
        sphere_grad = QRadialGradient(cx - base_r * 0.25, cy - base_r * 0.25, base_r * 0.85)
        sphere_grad.setColorAt(0.0, QColor(core_c1.red(), core_c1.green(), core_c1.blue(), 200))
        sphere_grad.setColorAt(0.5, QColor(core_c2.red(), core_c2.green(), core_c2.blue(), 150))
        sphere_grad.setColorAt(0.85, QColor(10, 12, 18, 220))
        sphere_grad.setColorAt(1.0, QColor(4, 5, 8, 250))
        p.setBrush(QBrush(sphere_grad))
        p.setPen(QPen(QColor(ring_c.red(), ring_c.green(), ring_c.blue(), 80), 1.5))
        p.drawEllipse(QPointF(cx, cy), base_r * 0.82, base_r * 0.82)

        # 6. Status and helper text at bottom of orb area
        sub_y = cy + base_r * 1.15
        p.setFont(QFont("Segoe UI", 9, QFont.Weight.DemiBold))
        status_sub = "VEDA AI • " + ("MUTED (TAP ORB TO UNMUTE)" if self.muted else self.state)
        p.setPen(QColor(220, 225, 235, 190) if not self.muted else QColor(255, 90, 90, 220))
        p.drawText(QRectF(0, sub_y, W, 22), Qt.AlignmentFlag.AlignCenter, status_sub)
...
class TaskCard(QFrame):
    def __init__(self, parent=None):
        ...
        self._plan_lbl = QLabel("Plan: Veda Echo will generate a task plan after you send a command.")
    ...
    def set_task(self, title: str, desc: str, percent: int):
        ...
        self._plan_lbl.setText("Plan: Veda Echo will generate a task plan after you send a command.")
...
class ChatBubble(QFrame):
    def __init__(self, role: str, name: str, text: str, stamp: str, attachments: list[dict] | None = None, parent=None, animate: bool = False):
        ...
        if role == "assistant":
            ...
            name_lbl = QLabel(name or "Veda Echo")
            name_lbl.setFont(QFont("Segoe UI", 9, QFont.Weight.Bold))
            name_lbl.setStyleSheet("color: #ffffff; background: transparent;")
...
class ConversationFeed(QScrollArea):
    ...
    def _ensure_empty_widget(self):
        ...
        lay.setSpacing(10)
        title = QLabel("Try asking Veda Echo")
...
class WorkspaceSidebar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        header.addWidget(self._title)
        header.addStretch()
        self._close_btn = QPushButton("VEDA ECHO")
    ...
    def append_log(self, text: str):
        ...
        if not raw:
            return
        low = raw.lower()
        if low.startswith(("you:", "veda echo:")):
            return
    ...
    def record_chat_event(self, event: object):
        ...
        if role == "user":
            ...
            self._active_conversation_id = convo_id
            self._feed.add_message("assistant", "Veda Echo", text, _fmt_time_stamp(stamp), attachments=attachments, animate=True)
            self._hide_memory_banner()
        elif role == "system":
...
class InlineChatWorkspace(QFrame):
    ...
    def _build_chat_tab(self) -> QWidget:
        ...
        self._footer_status = QLabel("Veda Echo is listening...")
    ...
    def record_chat_event(self, event: object):
        ...
        if role == "user":
            ...
            self._feed.add_message("assistant", "Veda Echo", text, stamp, attachments=attachments)
            self._hide_memories()
        elif role == "system":
...
class LauncherControlPanel(QDialog):
    def __init__(self, *, startup_workspace: bool = False, on_open=None, on_close=None,
        ...
        title = QLabel("VEDA ECHO CONTROL")
        title.setFont(QFont("Segoe UI", 13, QFont.Weight.Bold))
        title.setStyleSheet("color: #FFFFFF; background: transparent; letter-spacing: 1px;")
        lay.addWidget(title)
        ...
        self._restart_btn = mk_btn("Restart Veda Echo")
        self._quit_btn = mk_btn("Quit Veda Echo")
    ...
    def _hide_icon_confirm(self):
        ...
        flay.setSpacing(10)
        lbl = QLabel("Hide Veda Echo icon?")
...
class LogWidget(QScrollArea):
    ...
    def _parse(self, text: str) -> tuple[str, str, str]:
        ...
        if tl.startswith("veda echo:"):
            return "assistant", "Veda Echo", raw[len("Veda Echo:"):].strip()
        if tl.startswith("veda echo:"):
            return "assistant", "Veda Echo", raw[len("Veda Echo:"):].strip()
...
class FileDropZone(QWidget):
    ...
    def _browse(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select a file for Veda Echo", str(Path.home()),
            "All Files (*.*);;"
...
class SetupOverlay(QWidget):
    ...
    def _save_identity_and_next(self):
        identity.set_assistant_name(self._inp_ast.text().strip() or "Veda")
        identity.set_application_name(self._inp_app.text().strip() or "Veda Echo")
    ...
    def _build_stage3(self):
        ...
        self._intro_lines = []
        for txt in ["Identity confirmed.", "Hello.", "I'm Veda Echo.", "Ready whenever you are."]:
            lbl = QLabel(txt)
            lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
            if txt == "I'm Veda Echo.":
        ...
        self._launch_btn = QPushButton("Launch Veda Echo →")
    ...
    def _show_intro_final(self):
        """Show the Veda Echo intro sequence."""
        page = self._stack.widget(6)
        lay = page.layout()
...
class CommandBar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        logo_lbl = QLabel("V")  # Veda
        logo_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
        logo_lbl.setFont(QFont("Segoe UI", 10, QFont.Weight.Bold))
        ...
        self._input.setPlaceholderText("Ask Veda AI or type any command...")
...
class DeveloperModeDialog(QDialog):
    def __init__(self, parent=None, settings: dict | None = None):
        ...
        desc = QLabel("Pick a workspace folder Veda Echo should use when building websites or other workspace-based tasks.")
...
class ScanningOverlay(QWidget):
    ...
    def _generate_layout(self, pixmap):
        ...
        if pixmap is not None and cv2 is not None and np is not None:
            try:
...
class BootSequenceOverlay(QWidget):
    ...
    def paintEvent(self, event):
        try:
            ...
            bg_alpha = 1.0
            if self._time >= 0.80:
                bg_alpha = max(0.0, 1.0 - (self._time - 0.80) / 0.18)
            ...
            if 0.62 <= self._time < 0.80:
                # Spring compression
                t = (self._time - 0.62) / 0.18
                scale = 1.0 - math.sin(t * math.pi) * 0.05
            elif self._time >= 0.80:
                # Scale up
                t = (self._time - 0.80) / 0.18
                scale = 1.0 + math.pow(t, 4) * 20.0
                
            painter.scale(scale, scale)
            
            # Phase 1: Convergence (< 0.12)
            if self._time < 0.12:
                for d in self._dots:
                    t = (self._time - d['delay'] * 0.2) / (d['dur'] * 0.2)
                    if t < 0:
                        continue
                    if t > 1:
                        t = 1
            ...
            # Phase 2: Core (0.12 <= self._time < 0.28)
            if 0.12 <= self._time < 0.28:
                t = (self._time - 0.12) / 0.16
            ...
            # Phase 3 & beyond: Orbital Construction & Text (self._time >= 0.28)
            if self._time >= 0.28:
                t_draw = min(1.0, (self._time - 0.28) / 0.17)
                ring_rad = 140
                ...
                # Phase 4: Text (self._time >= 0.45)
                if self._time >= 0.45:
                    t_text = min(1.0, (self._time - 0.45) / 0.17)
                    ...
                    painter.drawText(QRectF(-ring_rad, -60, ring_rad*2, 60), Qt.AlignmentFlag.AlignCenter, "VEDA")
                    ...
                    painter.drawText(QRectF(-ring_rad, 0, ring_rad*2, 40), Qt.AlignmentFlag.AlignCenter, "AI - Lite")
                    
                    # Status message cycling
                    msg_idx = int(((self._time - 0.45) / 0.35) * len(self._status_messages))
                    msg_idx = min(len(self._status_messages)-1, max(0, msg_idx))
                ...
                # Phase 5: Pulse (0.62 <= self._time < 0.80)
                if 0.62 <= self._time < 0.80:
                    t_pulse = (self._time - 0.62) / 0.18
...
class MeetingOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._answer = QLabel("Veda Echo will show the live answer here.")
...
class FloatingLauncher(QWidget):
    ...
    def paintEvent(self, event):
        ...
        vfont = QFont("Segoe UI", 10, QFont.Weight.Bold)
        vfont.setLetterSpacing(QFont.SpacingType.AbsoluteSpacing, 1.2)
        painter.setFont(vfont)
        painter.drawText(QRectF(cx - 24, cy - 11, 48, 22), Qt.AlignmentFlag.AlignCenter, "VEDA")
    ...
    def _apply_state_style(self):
        self.setToolTip(f"Veda AI Widget\nStatus: {self._status_line}\n• Click: Quick Command Bar\n• Double-Click: Open App\n• Right-Click: Options / Voice\n• Drag: Move anywhere")
        self.update()
    ...
    def _show_menu(self, global_pos):
        ...
        actions = [
            ("🎙️ Voice Command", "voice_mode"),
            ("💬 Quick Command Bar", "command_bar"),
            ("🪟 Open Veda AI", "open_app"),
            ("🔇 Mute / Unmute Mic", "toggle_mute"),
            ("⚙️ Settings", "settings"),
            ("❌ Hide Widget", "hide_icon"),
        ]
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self.setWindowTitle("Veda AI - Lite")
    ...
    def _startup_enabled(self) -> bool:
        ...
        try:
            with winreg.OpenKey(
                ...
                try:
                    value, _ = winreg.QueryValueEx(key, "Veda Echo")
                    run_value = _startup_run_value()
                    if value != run_value:
                        winreg.SetValueEx(key, "Veda Echo", 0, winreg.REG_SZ, run_value)
                    return bool(value)
        ...
        try:
            with winreg.CreateKey(winreg.HKEY_CURRENT_USER, _startup_registry_key()) as key:
                if enabled:
                    ...
                    try:
                        winreg.DeleteValue(key, "Veda Echo")
                    except FileNotFoundError:
                        pass
            return True
        except Exception as e:
    ...
    def set_veda_connect_service(self, service):
        self._veda_connect = service
    ...
    def _build_header(self) -> QWidget:
        ...
        lay.addWidget(_badge("VEDA ECHO", C.PRI_DIM))
        lay.addStretch()

        mid = QVBoxLayout(); mid.setSpacing(1)
        title = QLabel("VEDA ECHO")
        title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        title.setFont(QFont("Courier New", 17, QFont.Weight.Bold))
        title.setStyleSheet(f"color: {C.PRI}; background: transparent;")
        mid.addWidget(title)
        sub = QLabel("Lite Edition by Rohan Vemula")
        sub.setAlignment(Qt.AlignmentFlag.AlignCenter)
    ...
    def _tick_clock(self):
        ...
        if hasattr(self, "_core_lbl") and self._core_lbl is not None:
            ...
            name = os.getenv("USERNAME") or os.getenv("USER") or "Rohan"
        ...
        if hasattr(self, "_core_status_lbl") and self._core_status_lbl is not None:
            self._core_status_lbl.setText("Veda is ready. SeekAI (glm-5.3-flash) · OpenRouter · Voice Connected · Memory Enabled")
    ...
    def _build_footer(self) -> QWidget:
        ...
        lay.addStretch()
        lay.addWidget(_fl("Rohan Vemula  ·  Veda Echo  ·  Open Source"))
        lay.addStretch()
        lay.addWidget(_fl("© Rohan Vemula", C.PRI_DIM))
        return w
    ...
    def _browse_attachment(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Attach a file to Veda Echo", str(Path.home()),
            "All Files (*.*);;"
            "Images (*.jpg *.jpeg *.png *.gif *.webp *.bmp *.svg);;"
            "Documents (*.pdf *.docx *.txt *.md *.pptx);;"
            "Data (*.csv *.xlsx *.json *.xml);;"
    ...
    def _toggle_mute(self):
        ...
        if hasattr(self, "orb") and self.orb is not None:
            self.orb.set_muted(self._muted)
        self._style_mute_btn()
        if self._muted:
        ...
        if hasattr(self, "_result_card") and low.startswith("you:"):
            ...
            if self.on_chat_event and user_msg:
                try:
                    ...
                    pass
        if hasattr(self, "_result_card") and low.startswith("veda echo:"):
    ...
    def notify_phone_connected(self):
        ...
        self._log_sig.emit("SYS: Phone connected to Veda Echo remote.")
    ...
    def _apply_state(self, state: str):
        ...
        if hasattr(self, "_task_card"):
            if state == "THINKING":
                self._task_card.set_task("Working on it...", "Veda Echo is processing your request.", 72)
            elif state == "SPEAKING":
                self._task_card.set_task("Responding...", "Veda Echo is speaking now.", 100)
            elif state == "MUTED":
                self._task_card.set_task("Microphone muted", "Voice input is paused.", 0)
            else:
                self._task_card.set_task("Ready", "Veda Echo is idle and ready.", 0)
        if hasattr(self, "_result_card"):
            if state == "THINKING":
    ...
    def _on_setup_done(self, key: str, or_key: str, os_name: str):
        try:
            ...
            if hasattr(self, '_floating_gesture_card'):
                self._floating_gesture_card.show()
            self._apply_state("LISTENING")
            self._log.append_log(f"SYS: Initialised. OS={os_name.upper()}. Veda Echo online.")
        except Exception as e:
    ...
    def _build_left_panel_modern(self) -> QWidget:
        ...
        title = QLabel("<span style='color:#ffb300;'>VEDA ECHO</span><br><span style='color:#ffffff;'>LITE</span>")
        title.setFont(QFont("Segoe UI", 16, QFont.Weight.Bold))
        ...
        self._nav_items["home"] = NavItem("Veda Echo Home", active=False, letter="H")
    ...
    def _build_center_panel_modern(self, face_path: str) -> QWidget:
        ...
        self.orb = VedaOrbWidget(self)
        self.orb.set_muted(getattr(self, "_muted", False))
        self.orb.mute_clicked.connect(self._toggle_mute)
        
        orb_wrap = QFrame()
        orb_wrap.setStyleSheet("background: transparent;")
        orb_lay = QVBoxLayout(orb_wrap)
        orb_lay.setContentsMargins(0, 0, 0, 0)
        orb_lay.setSpacing(0)
        orb_lay.addWidget(self.orb, alignment=Qt.AlignmentFlag.AlignCenter)

        command_row = QHBoxLayout()
        command_row.setSpacing(14)
        command_row.addWidget(self._command_card, alignment=Qt.AlignmentFlag.AlignVCenter)
        command_row.addWidget(orb_wrap, stretch=1)
        ...
        self._home_page = VedaHomePage()
        self._devices_page = VedaConnectDevicesPage(self)
    ...
    def _build_command_row(self) -> QHBoxLayout:
        ...
        self._input = QLineEdit()
        self._input.setPlaceholderText("Ask Veda Echo anything...")
...
class SystemConnectivitySidebar(QFrame):
    def __init__(self, controller=None, parent=None):
        ...
        self._mk_quick_action("Γå╗ Restart Veda Echo", QStyle.StandardPixmap.SP_BrowserReload, self._restart)
...
class SettingsHubPage(QWidget):
    def __init__(self, navigation_callback, parent=None):
        ...
        subtitle = QLabel("Select a section below to configure your Veda Echo environment.")
        ...
        cards_data = [
            ("Veda Echo Home", "Configure smart home integrations", "🏠", 1),
...
class SystemConnectivityPage(QWidget):
    ...
    def _build_left_column(self):
        ...
        # Instagram Connect
        ig_card = self._card("Instagram Connect", "Connect your personal Instagram account to allow Veda Echo to manage your DMs.")
        ...
        self._set_ast_name.textChanged.connect(lambda t: identity.set_assistant_name(t.strip() or "Veda"))
        ...
        self._set_app_name.textChanged.connect(lambda t: identity.set_application_name(t.strip() or "Veda Echo"))
        ...
        # Mobile connect
        mobile = self._card("Mobile Connect", "Connect your phone and control Veda Echo remotely.")
        ml = mobile.layout()
        ...
        # Startup
        startup = self._card("Startup", "Use Veda Echo with Windows startup preferences.")
        sl = startup.layout()
        self._startup_launch_btn = self._mk_toggle("Launch Veda Echo when Windows starts", bool(self._load_app_settings().get("show_workspace_on_startup", False)), self._toggle_startup_from_page)
        ...
        # Shortcuts & Pinning
        shortcuts = self._card("Shortcuts & Pinning", "Create shortcuts and pin Veda Echo to your Windows system.")
        ...
        # App Theme
        theme_card = self._card("App Theme", "Select the primary color theme for Veda Echo.")
        ...
        # Discord bot
        discord = self._card("Discord Bot", "Mirror Veda Echo between the app and your server.")
        dl = discord.layout()
        ...
        about = self._card("About Veda Echo", "Veda Echo information only.")
        ab = about.layout()
    ...
    def _build_quick_actions_box(self):
        ...
        lay = box.layout()
        actions = [
            ("Restart Veda Echo", QStyle.StandardPixmap.SP_BrowserReload, self._restart_app),
    ...
    def _test_discord_from_page(self):
        ...
        if self._ctrl() and hasattr(self._ctrl(), "_win"):
            ...
            self._discord_msg.setText("Connected as Veda Echo#9649" if self._discord_token.text().strip() else "Bot Offline")
    ...
    def refresh(self):
        ...
        if enabled and token:
            ...
            self._discord_msg.setText("Connected as Veda Echo#9649")
        elif token:
    ...
    def _create_desktop_shortcut_logic(self):
        try:
            ...
            desktop_dir.mkdir(parents=True, exist_ok=True)
            shortcut_path = desktop_dir / "Veda Echo - Premium.lnk"
            ...
            ps1_script = "\n".join([
                ...
                "$Shortcut.Description = 'Launch Veda Echo - Premium'",
    ...
    def _pin_app_to_taskbar_logic(self):
        try:
            ...
            if res.returncode == 0:
                return True, "Veda Echo has been pinned to your Taskbar!"
            else:
                return False, "Windows restricts programmatic taskbar pinning. Please right-click the 'Veda Echo - Premium.lnk' shortcut on your Desktop and select 'Pin to taskbar', or drag it directly onto your taskbar."
        except Exception as e:
...
class SmartDevicesSection(QFrame):
    def __init__(self, controller=None, parent=None):
        ...
        """)
        self._open_home_btn.clicked.connect(self._open_veda_home)
        ...
        empty_btn = QPushButton("Open Veda Echo Home")
        ...
        """)
        empty_btn.clicked.connect(self._open_veda_home)
...
class VedaUI:
    def __init__(self, face_path: str, size=None, *, show_immediately: bool = True):
        ...
        self._app.setApplicationDisplayName("Veda Echo")
        ...
        self._tray.setToolTip("Veda Echo")
    ...
    def set_veda_connect_service(self, service):
        self._win.set_veda_connect_service(service)
    ...
    def play_boot_sequence(self, finished_callback=None):
        ...
        overlay.start(device_name=device_name, greeting_name="Rohan")
    ...
    @property
    def orb(self):
        return getattr(self._win, "orb", None)
...
class VedaConnectDevicesPage(QFrame):
    def __init__(self, parent=None):
        super().__init__(parent)
        self._service = None
        ...
        self.setObjectName("VedaConnectDevicesPage")
        self.setStyleSheet(f"""
            QFrame#VedaConnectDevicesPage {{
                background: transparent;
                border: none;
            }}
            QLabel {{
        ...
        self._subtitle = QLabel("Everything connected to Veda.")
        ...
        w1_lay.addStretch(1)
        w1_title = QLabel("SCAN WITH PHONE CAMERA")
        ...
        w1_sub = QLabel("No app required! Scan to open Veda Mobile Web in your browser.")
        w1_sub.setAlignment(Qt.AlignmentFlag.AlignCenter)
        w1_sub.setFont(QFont("Segoe UI", 9))
        w1_sub.setStyleSheet("color: rgba(255,255,255,0.6);")
        w1_lay.addWidget(w1_sub)
        w1_lay.addSpacing(12)
        ...
        w1_lay.addSpacing(12)
        pin_title = QLabel("YOUR 6-DIGIT CONNECT PIN:")
        pin_title.setAlignment(Qt.AlignmentFlag.AlignCenter)
        pin_title.setFont(QFont("Segoe UI", 8, QFont.Weight.Bold))
        pin_title.setStyleSheet("color: rgba(255, 179, 0, 0.7); letter-spacing: 1.5px;")
        w1_lay.addWidget(pin_title)

        self._onb_code_lbl = QLabel("------")
        self._onb_code_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self._onb_code_lbl.setFont(QFont("Consolas", 22, QFont.Weight.Black))
        self._onb_code_lbl.setStyleSheet("color: #ffb300; letter-spacing: 8px;")
        w1_lay.addWidget(self._onb_code_lbl)

        self._onb_url_lbl = QLabel("")
        self._onb_url_lbl.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self._onb_url_lbl.setFont(QFont("Segoe UI", 9))
        self._onb_url_lbl.setStyleSheet("color: rgba(255,255,255,0.5);")
        w1_lay.addWidget(self._onb_url_lbl)
    ...
    def _service_obj(self):
        return self._service or getattr(self.parentWidget(), "_veda_connect", None)
    ...
    def _refresh_onboarding_offer(self):
        ...
        try:
            ...
            self._onboarding_offer = dict(service.create_pairing_offer(device_name="Veda Connect Mobile", platform="web"))
            ...
            host = str(self._onboarding_offer.get("host") or "127.0.0.1")
            port = int(self._onboarding_offer.get("port") or 8765)
            web_url = f"http://{host}:{port}/"
            if hasattr(self, "_onb_url_lbl"):
                self._onb_url_lbl.setText(web_url)
            ...
            if qrcode is not None:
                ...
                self._onb_qr_label.setText(web_url)
...
class VedaUI:
    def __init__(self, face_path: str, size=None, *, show_immediately: bool = True):
        ...
        self._app_settings_cache: dict | None = None
        self._launcher.single_clicked.connect(self._toggle_command_bar)
        self._launcher.double_clicked.connect(self.show_main)
        self._launcher.action_requested.connect(self._handle_launcher_action)
        self._launcher.position_changed.connect(self._save_launcher_position)
        self._command_bar.submitted.connect(self._submit_command)
        self._command_bar.attach_clicked.connect(self._browse_attachment)
    ...
    def _handle_launcher_action(self, action: str):
        ...
        if action == "open_app":
            ...
            if getattr(self._win, "_muted", False):
                self._win._toggle_mute()
            if hasattr(self._win, "_toggle_mic"):
                self._win._toggle_mic()
            self._command_bar.show_near(self._launcher)
        elif action == "command_bar":
            self._toggle_command_bar()
        elif action == "toggle_mute":
            self._toggle_mute()
        elif action == "settings":
            self.show_main()
            if hasattr(self._win, "_btn_settings"):
                self._win._btn_settings.click()
        elif action == "open_workspace":
            self._show_workspace_sidebar()
        elif action == "close_workspace":
            self._close_workspace_sidebar()
