...
def _default_config_path(base_dir: Path) -> Path:
    return Path(base_dir) / "config" / "veda_connect.json"


def _default_registry_path(base_dir: Path) -> Path:
    return Path(base_dir) / "config" / "veda_connect" / "devices.json"


@dataclass(slots=True)
class VedaGatewayConfig:
    host: str = "0.0.0.0"
    port: int = 8765
    enabled: bool = True
    advertise: bool = True
    service_name: str = "_VEDA._tcp.local."
    ...
    @classmethod
    def load(cls, base_dir: Path, override: dict[str, Any] | None = None) -> "VedaGatewayConfig":
        ...
        return cls(
            ...
            service_name=str(data.get("service_name", "_VEDA._tcp.local.")),
...
class VedaGateway:
    def __init__(self, base_dir: Path, config: VedaGatewayConfig | None = None):
        self.base_dir = Path(base_dir)
        self.config = config or VedaGatewayConfig.load(self.base_dir)
    ...
    async def disconnect_device(self, device_or_target: str, *, reason: str = "Disconnected by Veda") -> dict[str, Any]:
        query = str(device_or_target or "").strip()
        matches = self.device_manager.resolve(query)
        record = None
    ...
    def _build_app(self) -> FastAPI:
        from .mobile_web import MOBILE_WEB_HTML
        app = FastAPI(docs_url=None, redoc_url=None)

        @app.get("/", response_class=HTMLResponse)
        @app.get("/connect", response_class=HTMLResponse)
        async def mobile_home():
            return HTMLResponse(content=MOBILE_WEB_HTML, status_code=200)
        ...
        async def verify_pin(request: Request):
            try:
                ...
                offer = self.pairing_manager.get_offer_by_code(pin)
                if offer is not None:
                    return {"ok": True, "token": offer.pairing_token}
                for t, off in list(self.pairing_manager._offers.items()):
                    if off.pairing_code == pin:
                        return {"ok": True, "token": off.pairing_token}
                return JSONResponse({"ok": False, "error": "Invalid or expired PIN. Please check your screen."}, status_code=400)
        ...
        async def execute_mobile_command(request: Request):
            try:
                ...
                if self.on_chat_message:
                    res = self.on_chat_message(cmd)
                    if isinstance(res, str) and res:
                        reply = res
                    else:
                        reply = f"Command '{cmd}' executed by Veda AI."
                else:
                    reply = f"Command '{cmd}' received by Veda AI."
                
                self._append_log("MOBILE_COMMAND", command=cmd, reply=reply[:80])
                return {"ok": True, "reply": reply}
            except Exception as e:
                return JSONResponse({"ok": False, "error": str(e)}, status_code=500)

        @app.get("/api/system_status")
        async def mobile_system_status():
            try:
                import psutil
                ...
                return {"ok": True, "cpu": cpu, "ram": ram, "battery": battery_percent}
            except Exception:
                return {"ok": True, "cpu": 0, "ram": 0, "battery": None}
                    ...
                    if msg_type == ProtocolTypes.HELLO:
                        ...
                        await websocket.send_json(
                            build_message(
                                ...
                                {
                                    "pending_id": pending_id,
                                    "message": "Pairing request received. Awaiting user approval in Veda.",
    ...
    async def serve(self) -> None:
        ...
        if self.config.advertise:
            advertised = self.discovery.start(host=self.config.host, port=self.config.port, properties={"service": "veda", "version": "1"})
