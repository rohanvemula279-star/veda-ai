...
from .gateway.server import VedaGateway, VedaGatewayConfig


@dataclass(slots=True)
class VedaConnectService:
    """Small runtime wrapper around the gateway server."""

    base_dir: Path
    config: VedaGatewayConfig | None = None
    gateway: VedaGateway = field(init=False)
    _thread: threading.Thread | None = field(init=False, default=None)
    _started: bool = field(init=False, default=False)

    def __post_init__(self) -> None:
        self.base_dir = Path(self.base_dir)
        self.gateway = VedaGateway(self.base_dir, self.config)
    ...
    def start_background(self) -> None:
        ...
        self._thread = threading.Thread(target=_runner, name="VedaConnectGateway", daemon=True)
        self._thread.start()
        self._started = True
    ...
    async def disconnect_device(self, target: str, *, reason: str = "Disconnected by Veda") -> dict[str, Any]:
...
_SERVICE: VedaConnectService | None = None


def get_service(base_dir: str | Path) -> VedaConnectService:
    global _SERVICE
    if _SERVICE is None:
        _SERVICE = VedaConnectService(Path(base_dir))
    return _SERVICE
