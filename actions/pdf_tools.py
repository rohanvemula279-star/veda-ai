"""
pdf_tools.py - Veda AI PDF support
...
def _pdf_story_from_blocks(blocks: list[dict], pdf, styles):
    ...
    for block in blocks:
        ...
        if kind == "heading":
            ...
            story.append(pdf["Paragraph"](text, styles.get(f"veda_h{min(max(level, 1), 3)}", styles["veda_h1"])))
            story.append(pdf["Spacer"](1, 0.1 * pdf["inch"]))
        elif kind == "bullet":
            story.append(pdf["Paragraph"](f"- {text}", styles["veda_body"]))
            story.append(pdf["Spacer"](1, 0.03 * pdf["inch"]))
        elif kind == "numbered":
            story.append(pdf["Paragraph"](f"{number_index}. {text}", styles["veda_body"]))
            number_index += 1
            story.append(pdf["Spacer"](1, 0.06 * pdf["inch"]))
        elif kind == "table":
            rows = block.get("text") or []
            table_data = [[pdf["Paragraph"](cell, styles["veda_table_cell"]) for cell in row.split(" | ")] for row in rows]
            if table_data:
            ...
            story.append(pdf["Paragraph"](text, styles["veda_body"]))
...
def create_pdf(parameters: dict, player=None) -> str:
    ...
    output_path = _resolve_output_path(parameters.get("output_path"), title, ".pdf", "veda_ai_output")
    ...
    doc = pdf["SimpleDocTemplate"](
        ...
        title=title,
        author=parameters.get("author") or "Rohan",
    ...
    if title or subtitle:
        ...
        story.append(pdf["Paragraph"]("<hr/>", styles["veda_body"]))
    ...
    if action == "create_letter":
        ...
        signature = (parameters.get("signature") or parameters.get("author") or "Rohan").strip()
        body_text = parameters.get("body") or parameters.get("content") or ""

        story.append(pdf["Paragraph"](date_value, styles["veda_body"]))
        story.append(pdf["Spacer"](1, 0.08 * pdf["inch"]))
        if recipient:
            story.append(pdf["Paragraph"](recipient, styles["veda_body"]))
            story.append(pdf["Spacer"](1, 0.06 * pdf["inch"]))
        story.append(pdf["Paragraph"](salutation, styles["veda_body"]))
        story.append(pdf["Spacer"](1, 0.08 * pdf["inch"]))
        for para in _normalize_list(parameters.get("paragraphs")) or [p.strip() for p in re.split(r"\n\s*\n", str(body_text)) if p.strip()]:
            story.append(pdf["Paragraph"](str(para), styles["veda_body"]))
            story.append(pdf["Spacer"](1, 0.08 * pdf["inch"]))
        story.append(pdf["Spacer"](1, 0.18 * pdf["inch"]))
        story.append(pdf["Paragraph"](closing, styles["veda_body"]))
        story.append(pdf["Spacer"](1, 0.3 * pdf["inch"]))
        story.append(pdf["Paragraph"](signature, styles["veda_body"]))
    else:
