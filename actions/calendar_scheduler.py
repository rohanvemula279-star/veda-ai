...
"""
Calendar and Schedule Management for Veda AI.
...
def calendar_scheduler(
    ...
    if action in ("add", "add_event", "create", "new"):
        ...
        ics_lines = [
            "BEGIN:VCALENDAR",
            "VERSION:2.0",
            "PRODID:-//Veda AI//Calendar Scheduler//EN",
        ]
        for ev in events:
            try:
                ...
                ics_lines.extend([
                    "BEGIN:VEVENT",
                    f"UID:{ev.get('id', uuid.uuid4())}@veda.ai",
        ...
        desktop_ics = Path.home() / "Desktop" / "veda_calendar.ics"
