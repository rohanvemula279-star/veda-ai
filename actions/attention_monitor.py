...
def _speak_edge_native(text: str) -> None:
    ...
    audio_path = os.path.join(tempfile.gettempdir(), f"veda_edge_tts_{uuid.uuid4().hex}.mp3")
    try:
    ...
    player_alias = f"veda_tts_{uuid.uuid4().hex}"
    try:
        ...
        try:
            ...
            if _sapi_speaker is not None:
                try:
                    ...
                    _sapi_speaker.Speak(text, 0)
                    spoken = True
                except Exception as e:
                    print(f"[AttentionMonitor] SAPI speak failed: {e}")
                    spoken = False
            ...
            print(f"[AttentionMonitor] TTS worker error: {e}")
