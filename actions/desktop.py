...
def _execute_generated_code(code: str, player=None) -> str:
    ...
    try:
        exec(compile(code, "<veda_desktop>", "exec"), sandbox)
...
def organize_desktop(mode: str = "by_type", target_path: Path | str | None = None) -> str:
    desktop = Path(target_path).expanduser() if target_path else _get_desktop()
    ...
    result = f"{desktop.name} organized ({mode}): {len(moved)} files moved."
    if moved:
        result += "\n" + "\n".join(moved[:8])
        if len(moved) > 8:
...
def desktop_control(
    ...
    try:
        if action == "wallpaper":
            ...
            target_path = params.get("path") or params.get("folder")
            task_desc = str(params.get("task", "") or params.get("description", "") or "").lower()
            if not target_path and "download" in task_desc:
                target_path = Path.home() / "Downloads"
            return organize_desktop(params.get("mode", "by_type"), target_path=target_path)
