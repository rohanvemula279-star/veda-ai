...
def _safe_screenshot_path(requested: str | None) -> Path:
    fallback = Path.home() / "Desktop" / "veda_screenshot.png"
    if not requested:
        return fallback
    try:
...
def _type(text: str, interval: float = 0.03, app: str | None = None) -> str:
    _require_pyautogui()
    if app:
        _focus_window(app)
        time.sleep(0.3)
    ...
    # Use clipboard paste for reliable Unicode/special chars or fast typing
    if _PYPERCLIP and (len(text) > 8 or "\n" in text or any(ord(c) > 127 for c in text)):
        pyperclip.copy(text)
        time.sleep(0.05)
    ...
    if os_name == "windows":
        try:
            ...
            if found_hwnds:
                ...
                time.sleep(0.3)
                return f"Focused window: {found_hwnds[0][1]}"
        except Exception:
            pass
...
def _screen_find(description: str) -> tuple[int, int] | None:
    try:
        ...
        try:
            ...
            print(f"[ComputerControl] Gemini vision fallback: {e}")

        # 2. Fallback to llm_client
        if not text:
            try:
                ...
                print(f"[ComputerControl] llm_client vision failed: {e}")
        ...
        if text and "NOT_FOUND" not in text.upper():
            ...
            if match:
                ...
                return min(max(0, x_coord), w - 1), min(max(0, y_coord), h - 1)
    except Exception as e:
...
def computer_control(
    ...
    try:
        ...
        if action in ("type", "write"):
            target_app = params.get("app") or params.get("window") or params.get("app_name") or params.get("target")
            return _type(params.get("text", "") or params.get("content", ""), app=target_app)
