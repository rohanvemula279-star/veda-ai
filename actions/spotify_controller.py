...
"""
Universal Music & Spotify Controller for Veda AI.
...
def spotify_controller(
    ...
    if action in ("play", "pause", "toggle", "playpause", "resume"):
        ...
        # If Desktop Spotify App exists, launch it and trigger playback
        if spotify_app:
            try:
                os.startfile(f"spotify:search:{encoded}")
                time.sleep(2.0)

                # Focus Spotify window if possible
                try:
                ...
                # Navigate to and play top search result
                try:
                    import pyautogui
                ...
                print(f"[Music] Spotify desktop search error: {exc}")
        ...
        if direct_url:
            ...
            if player:
                try:
                    player.write_log(f"Veda AI: Playing '{query}' in Google Chrome")
                except Exception:
                    pass
