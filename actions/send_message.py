...
def _send_instagram(receiver: str, message: str) -> str:
    """
    Sends an Instagram DM via API or browser (instagram.com/direct/new/).
    Guarantees direct navigation to Messages rather than Reels or Explore.
    """
    # 1. First attempt: Direct API via instagrapi if user configured Instagram credentials
    try:
        from actions.instagram_chat import _client as ig_client
        if ig_client:
            user_id = ig_client.user_id_from_username(receiver)
            if user_id:
                ig_client.direct_send(message, user_ids=[user_id])
                return f"Message sent to {receiver} via Instagram Direct Message."
    except Exception as e:
        print(f"[SendMessage] Direct IG API attempt: {e}")

    # 2. Browser Automation: Open direct new message URL directly
    try:
        import webbrowser
        # Open Instagram Direct New Message directly — NEVER navigates to Reels!
        webbrowser.open("https://www.instagram.com/direct/new/")
        time.sleep(4.5)

        # Focus recipient search and paste recipient username
        pyperclip.copy(receiver)
        pyautogui.hotkey("ctrl", "v")
        time.sleep(1.8)

        # Select first matching recipient
        pyautogui.press("tab")
        time.sleep(0.2)
        pyautogui.press("space")
        time.sleep(0.4)

        # Navigate to Chat / Next button
        for _ in range(3):
            pyautogui.press("tab")
            time.sleep(0.12)
        pyautogui.press("enter")
        ...
        # Type / paste the message and hit Enter
