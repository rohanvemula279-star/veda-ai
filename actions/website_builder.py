...
def _fallback_spec(parameters: dict[str, Any]) -> dict[str, Any]:
    ...
    site_name = _sanitize_text(
        str(parameters.get("site_name") or parameters.get("title") or "Veda Project"),
        "Veda Project",
    )
...
"""
        js = """
const STORAGE_KEY = 'veda.groceries.items';
...
Hard rules:
- Do not mention Veda, Codex, or any assistant branding in the site content.
...
def _normalize_page(page: dict[str, Any], site_name: str, brief: str, idx: int) -> dict[str, Any]:
    ...
    hero_data = {
        "eyebrow": _sanitize_text(str(hero.get("eyebrow") or ("Built with Veda" if idx == 0 else "Project Page")), "Built with Veda"),
    ...
    return f"""
      <div class="workspace-shell">
        <aside class="workspace-sidebar">
          <div class="sidebar-brand">Veda Workspace</div>
...
<body id="top">
  ...
  <header class="topbar">
    <div class="brand">
      ...
      <div>
        ...
        <span>{_html(tagline or "Generated with Veda")}</span>
      </div>
    </div>
  ...
  <footer class="footer reveal">
    <div>
      <strong>{_html(spec.get("site_name") or "Website")}</strong>
      <p>{_html(tagline or "Generated with Veda")}</p>
    </div>
...
<body id="top">
  ...
  <main class="layout-{_html(layout_mode)}">
    <section class="hero hero--{_html(layout_mode)} reveal">
      <div class="hero-copy">
        <p class="eyebrow">{_html(hero.get("eyebrow") or "Built with Veda")}</p>
        <h1>{_html(hero.get("title") or title)}</h1>
        <p class="lead">{_html(hero.get("subtitle") or tagline)}</p>
        <div class="hero-actions">
