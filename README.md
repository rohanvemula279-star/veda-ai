<div align="center">
  <img src="assets/Veda_Lite_Logo.png" alt="Veda Echo" width="260" />
...
Veda Echo is a premium Windows desktop assistant that combines voice and text control with automated workflows, screen-aware intelligence, and rich content generation.

Designed for advanced desktop productivity, Veda Echo delivers:
...
- Remote control via Discord and Veda Connect
...
- Wake-word support for “Veda Echo” and responsive assistant activation
...
- Device pairing and remote routing through Veda Connect
...
- Veda Connect for device discovery and command routing
...
```powershell
git clone https://github.com/titechprabhasolutions/Veda-AI---Lite.git
cd "Veda AI - Lite"
```
...
- `config/veda_connect.json` — device pairing, gateway, and discovery settings
...
- `veda_connect/` — local gateway, pairing, and remote routing
...
Extend Veda Echo with custom Python plugins by adding files to `plugins/`.

Supported hooks:

- `on_veda_created(veda)` — called when the assistant instance is initialized
- `on_startup(veda)` — called after startup when plugins are registered
- `on_text_command(text, source, veda=None)` — called for each incoming text command; return `True` to indicate the command was handled
...
> Preserve attribution and keep credentials secure when building on top of Veda Echo.
