...
from actions.veda_connect      import (
...
try:
    from veda_connect.service import get_service as get_veda_connect_service
except Exception:
    try:
        from veda_connect.service import get_service as get_veda_connect_service
    except Exception:
        get_veda_connect_service = None
...
STARTUP_LOG     = Path(os.environ.get("LOCALAPPDATA", str(BASE_DIR))) / "VedaAI" / "startup.log"
...
def _ensure_desktop_shortcut() -> None:
    ...
    try:
        ...
        shortcut_path = desktop_dir / "Veda AI - Lite.lnk"
        script_path = BASE_DIR / "main.py"
        icon_path = BASE_DIR / "assets" / "Veda_Lite_Logo.ico"
        ...
        ps1_script = "\n".join([
            ...
            "$Shortcut.Description = 'Launch Veda AI - Lite'",
...
def _load_system_prompt() -> str:
    try:
        ...
        base_prompt = (
            "You are Veda Echo, a calm, direct, and professional AI assistant. "
    ...
    try:
        ...
        ast_name = identity.get_assistant_name() or "Veda Echo"
...
def _gemini_text_reply(prompt: str) -> str:
    ...
    system_prompt = (
        "You are Veda Echo, a concise, helpful desktop assistant. "
...
def _ig_gemini_reply(username: str, text: str) -> str:
    system_prompt = (
        "You are Veda Echo, an AI personal assistant acting on behalf of your user. "
...
def _clipboard_gemini_reply(text: str) -> str:
    system_prompt = (
        "You are Veda Echo, a witty and helpful AI assistant. "
...
def _wakeword_detected(text: str) -> bool:
    ...
    if compact in phrases or any(p in compact for p in phrases):
        return True
    return any(word in {"veda echo", "hey", "hi", "hello"} for word in words)
...
def _update_memory_async(user_text: str, veda_text: str) -> None:
    global _last_memory_input

    user_text   = (user_text   or "").strip()
    veda_text = (veda_text or "").strip()
    ...
    try:
        api_key = _get_api_key()
        if not should_extract_memory(user_text, veda_text, api_key):
            return
        data = extract_memory(user_text, veda_text, api_key)
        if data:
            update_memory(data)
...
TOOL_DECLARATIONS = [
    ...
    },
    {
        ...
        "description": (
            "Lists devices connected to Veda Connect. Use when the user asks what devices are connected, "
    ...
    },
    {
        ...
        "description": (
            "Routes a Veda Connect command to a paired device through the gateway. "
    ...
    },
    {
        ...
        "description": (
            "Creates or approves Veda Connect pairing. Use to generate a QR code / pairing code for a new device, "
    ...
    },
    {
        ...
        "description": (
            "Disconnects a device from Veda Connect and marks it offline. "
    ...
    },
    {
        ...
        "description": (
            "Controls the web browser. Use for: autonomous multi-step web tasks (Astra-6 style), "
            "opening websites, searching the web, navigating pages, clicking elements by ID, filling forms, scrolling, "
            "extracting content, and any web automation task."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "action":      {"type": "STRING", "description": "autonomous_task | go_to | navigate | search | click | type | scroll | extract | fill_form | smart_click | smart_type | get_text | get_tree | press | back | forward | refresh | open_tab | new_tab | switch_tab | list_tabs | close"},
                "goal":        {"type": "STRING", "description": "High-level goal for autonomous_task (e.g. 'find cheapest flights from NYC to Paris', 'compare prices for Sony headphones', 'fill reservation form')"},
                "element_id":  {"type": "STRING", "description": "Element ID [id: N] from DOM accessibility tree for click/type/extract"},
                "url":         {"type": "STRING", "description": "URL for go_to / navigate action"},
    ...
    },
    {
        ...
        "description": (
            ...
            "Veda Echo automatically infers the best visual style from the topic, searches for a matching online template when available, "
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "theme": {
                    "type": "STRING",
                    "description": "Optional presentation theme or visual direction such as neon, corporate, luxury, academic, sunset, or creative. If omitted, Veda Echo infers the best style automatically."
                },
                "outline": {
                    "type": "STRING",
    ...
    },
    {
        ...
        "close the assistant, say goodbye, or stop Veda Echo. "
    ...
    },
    {
        ...
        "parameters": {
            ...
            "properties": {
                ...
                "value": {"type": "STRING", "description": "Concise value in English (e.g. Rohan, pizza, older sister)"},
            },
...
def _clean_command_text(text: str) -> str:
    ...
    prefixes = [
        r"^(?:hey|hi|hello|heyy|heyy\s+hi|yo|ok|okay|please|can\s+you|could\s+you|would\s+you|veda|veda|veda\s+echo|veda\s+ai)[\s,!:]+",
        r"^(?:can\s+you\s+please|could\s+you\s+please|please\s+can\s+you)[\s,!:]+",
        r"^(?:please\s+open|can\s+you\s+open|could\s+you\s+open)\s+",
    ]
...
class VedaLive:
    ...
    def __init__(self, ui: VedaUI, dashboard=None, dashboard_started: bool = False, enable_dashboard: bool = True):
        self.ui             = ui
    ...
    def _handle_direct_action(self, text: str, source: str = "local") -> bool:
        cleaned = _clean_command_text(text)
        lower = cleaned.lower().strip()
        orig_lower = text.lower().strip()

        # 1. YouTube playback: "play [query]", "listen to [query]"
        for prefix in ("play on youtube", "play song", "play video", "play", "listen to"):
            if lower.startswith(prefix + " "):
                query = cleaned[len(prefix):].strip()
                if query and not any(query.lower().startswith(w) for w in ["with ", "game", "file", "on ", "in "]):
                    self.ui.write_log(f"You: {text}")
                    msg = f"Playing {query} on YouTube."
                    self.ui.write_log(f"Veda AI: {msg}")
                    self.speak(msg)
                    try:
                        self.ui.begin_task_workspace(text, ["Search YouTube", "Launch video in browser"], source=source or "local")
                    except Exception:
                        pass
                    threading.Thread(
                        target=lambda: youtube_video({"action": "play", "query": query}, player=self.ui),
                        daemon=True,
                    ).start()
                    return True

        # 2. Close applications & windows: "close [app]", "kill [app]", "exit [app]", "close window"
        for prefix in ("close app", "kill app", "exit app", "quit app", "terminate app", "close", "kill", "exit", "quit", "terminate"):
            if lower == prefix or lower.startswith(prefix + " "):
                target = cleaned[len(prefix):].strip()
                if not target and lower in ("close", "close window", "exit", "quit"):
                    target = "window"
                if target and not any(target.lower() == w for w in ["veda", "veda", "assistant", "ai"]):
                    self.ui.write_log(f"You: {text}")
                    from actions.open_app import close_app
                    msg = close_app(target, player=self.ui)
                    self.ui.write_log(f"Veda AI: {msg}")
                    self.speak(msg)
                    return True

        # 3. Open / launch applications or websites: "open [app]", "launch [app]", "go to [site]"
        for prefix in ("open", "launch", "start", "run", "go to"):
            if lower.startswith(prefix + " "):
                app_target = cleaned[len(prefix):].strip()
                # If this is a compound / multi-step action, let planner/agent handle it
                is_compound = any(f" {conj} " in app_target.lower() for conj in ("and", "then", "and then", "after that")) or any(
                    act in app_target.lower() for act in (" type ", " write ", " search ", " and open ", " and close ")
                )
                if is_compound:
                    break
                ...
                if app_target:
                    self.ui.write_log(f"You: {text}")
                    msg = f"Opening {app_target}."
                    self.ui.write_log(f"Veda AI: {msg}")
                    self.speak(msg)
                    try:
                        self.ui.begin_task_workspace(text, [f"Resolve {app_target}", "Execute launcher"], source=source or "local")
                    except Exception:
                        pass
                    def _do_open():
                        res = open_app({"app_name": app_target}, player=self.ui)
                        try:
                            self.ui.finish_task_workspace(res, "Opened.", 100)
                        except Exception:
                            pass
                    threading.Thread(target=_do_open, daemon=True).start()
                    return True

        # 3b. Direct Typing & Keyboard Input: "type [text]", "type [text] in/on [app]", "write [text] in/on [app]"
        for prefix in ("type ", "write "):
            if lower.startswith(prefix):
                # Compound command check
                if any(f" {conj} " in lower for conj in ("then", "and then", "after that")):
                    break
                content = cleaned[len(prefix):].strip()
                target_app = None
                text_to_type = content
                match_target = re.search(r"^(.*?)\s+(?:on|in|into)\s+([a-zA-Z0-9_\s]+)$", content, re.IGNORECASE)
                if match_target and match_target.group(1).strip():
                    possible_app = match_target.group(2).strip()
                    if any(possible_app.lower().startswith(a) or a in possible_app.lower() for a in ("notepad", "word", "document", "chat", "browser", "terminal", "cmd", "code", "text")):
                        text_to_type = match_target.group(1).strip()
                        target_app = possible_app

                if text_to_type:
                    self.ui.write_log(f"You: {text}")
                    msg = f"Typing into {target_app}." if target_app else "Typing text."
                    self.ui.write_log(f"Veda AI: {msg}")
                    self.speak(msg)
                    try:
                        self.ui.begin_task_workspace(text, [f"Focus {target_app or 'active window'}", "Type text"], source=source or "local")
                    except Exception:
                        pass
                    def _do_type():
                        from actions.computer_control import computer_control, _focus_window
                        from actions.open_app import _is_running, open_app
                        if target_app:
                            if not _is_running(target_app):
                                open_app({"app_name": target_app}, player=self.ui)
                                time.sleep(1.0)
                            _focus_window(target_app)
                            time.sleep(0.3)
                        res = computer_control({"action": "type", "text": text_to_type, "app": target_app}, player=self.ui)
                        try:
                            self.ui.finish_task_workspace(res, "Typed.", 100)
                        except Exception:
                            pass
                    threading.Thread(target=_do_type, daemon=True).start()
                    return True

        # 4. Real-time Clock, Date, Day inquiries
        if any(lower == q or lower.startswith(q + " ") for q in (
            "what time is it", "what's the time", "current time", "tell me the time", "time now",
            "what is the time", "what time", "what date is it", "what is the date", "what's the date",
            "today's date", "what is today's date", "what day is today", "what day is it", "what year is this"
        )):
            self.ui.write_log(f"You: {text}")
            ...
            msg = f"It is {time_str} on {date_str}."
            self.ui.write_log(f"Veda AI: {msg}")
            self.speak(msg)
            return True

        # 5. Battery and System Resource Inquiries
        if any(k in lower for k in ("battery status", "battery percentage", "how much battery", "is my laptop charging", "system performance", "ram usage", "cpu usage")):
            self.ui.write_log(f"You: {text}")
            try:
                import psutil
                ...
                if batt:
                    plugged = "plugged in" if batt.power_plugged else "on battery power"
                    msg = f"Your battery is at {batt.percent}% and {plugged}. CPU usage is {cpu}%, and RAM usage is {ram}%."
                else:
                    msg = f"System is currently running at {cpu}% CPU usage and {ram}% RAM usage."
            except Exception as e:
                msg = f"Could not read battery metrics: {e}"
            self.ui.write_log(f"Veda AI: {msg}")
            self.speak(msg)
            return True

        # 6. Media and Music Playback Controls
        if lower in ("pause", "pause music", "pause song", "pause video", "stop music", "resume", "resume music", "resume song", "play music", "resume playback"):
            self.ui.write_log(f"You: {text}")
            try:
                import pyautogui
                pyautogui.press("playpause")
                msg = "Toggled media playback."
            except Exception:
                msg = "Done."
            self.ui.write_log(f"Veda AI: {msg}")
            self.speak(msg)
        ...
        if lower in ("previous song", "previous track", "prev song", "prev track"):
            self.ui.write_log(f"You: {text}")
            try:
                import pyautogui
        ...
        if lower in ["unmute", "unmute volume", "unmute sound"]:
            self.ui.write_log(f"You: {text}")
            try:
                import pyautogui
                pyautogui.press("volumemute")
        ...
        if "volume down" in lower or "decrease volume" in lower or "lower volume" in lower:
            self.ui.write_log(f"You: {text}")
            try:
                import pyautogui
                for _ in range(5):
        ...
        # 8. Windows Desktop & Window Management
        if lower in ("minimize all", "show desktop", "minimize all windows", "hide windows"):
            self.ui.write_log(f"You: {text}")
            try:
                import pyautogui
        ...
        if lower in ("lock computer", "lock screen", "lock pc", "lock workstation"):
            self.ui.write_log(f"You: {text}")
            try:
                import ctypes
        ...
        if lower in ("maximize window", "maximize this"):
            self.ui.write_log(f"You: {text}")
            try:
                import pyautogui
                pyautogui.hotkey("win", "up")
                msg = "Maximized window."
            except Exception:
                msg = "Done."
            self.ui.write_log(f"Veda AI: {msg}")
            self.speak(msg)
        ...
        if lower in ("minimize window", "minimize this"):
            self.ui.write_log(f"You: {text}")
            try:
                import pyautogui
                pyautogui.hotkey("win", "down")
                msg = "Minimized window."
            except Exception:
                msg = "Done."
            self.ui.write_log(f"Veda AI: {msg}")
            self.speak(msg)
        ...
        if lower in ("what is on my clipboard", "read clipboard", "read my clipboard", "clipboard content", "paste clipboard"):
            self.ui.write_log(f"You: {text}")
            try:
                ...
                root.destroy()
                msg = f"Clipboard content: \"{clip[:150]}\"" if clip else "Clipboard is empty."
            except Exception:
                msg = "Could not read clipboard."
            self.ui.write_log(f"Veda AI: {msg}")
            self.speak(msg)
            return True

        if lower.startswith("copy to clipboard ") or lower.startswith("copy this to clipboard "):
            self.ui.write_log(f"You: {text}")
            to_copy = cleaned.split("clipboard", 1)[1].strip()
            try:
                import tkinter as tk
                root = tk.Tk()
                root.withdraw()
                ...
                root.update()
                root.destroy()
                msg = f"Copied to clipboard: \"{to_copy[:60]}\""
            except Exception as e:
                msg = f"Copy failed: {e}"
            self.ui.write_log(f"Veda AI: {msg}")
            self.speak(msg)
        ...
        # 10. Web Search & Wikipedia
        for prefix in ("search google for", "search web for", "google for", "google"):
            if lower.startswith(prefix + " "):
                query = cleaned[len(prefix):].strip()
                if query:
                    self.ui.write_log(f"You: {text}")
        ...
        for prefix in ("search wikipedia for", "wikipedia for", "wikipedia"):
            if lower.startswith(prefix + " "):
                query = cleaned[len(prefix):].strip()
                if query:
                    self.ui.write_log(f"You: {text}")
                    msg = f"Searching Wikipedia for {query}."
                    self.ui.write_log(f"Veda AI: {msg}")
                    self.speak(msg)
                    import webbrowser
                    import urllib.parse
        ...
        # 11. Weather inquiries
        if lower in ["weather", "what is the weather", "what's the weather", "current weather", "how is the weather", "weather report"] or (lower.startswith("weather in ") and len(lower) < 45):
            self.ui.write_log(f"You: {text}")
            city = "LB Nagar, Hyderabad"
            if lower.startswith("weather in "):
                city = text[11:].strip() or city
            threading.Thread(
                target=lambda: weather_action({"city": city}, player=self.ui),
                daemon=True,
            ).start()
            return True

        # 12. Screenshot / screen capture
        if lower in ["take a screenshot", "take screenshot", "capture screen", "screenshot"]:
            self.ui.write_log(f"You: {text}")
            try:
                import pyautogui
                desktop = Path.home() / "Desktop"
                shot_path = desktop / f"screenshot_{int(time.time())}.png"
                pyautogui.screenshot(str(shot_path))
                msg = "Screenshot saved to your Desktop."
                self.ui.write_log(f"Veda AI: {msg}")
                self.speak(msg)
            except Exception as e:
                self.ui.write_log(f"ERR: Screenshot failed: {e}")
            return True

        # 13. Desktop clean / organize
        if lower in ["clean desktop", "organize desktop", "tidy desktop", "clean up desktop"]:
            self.ui.write_log(f"You: {text}")
            try:
                msg = "Cleaning and organizing your desktop files."
                self.ui.write_log(f"Veda AI: {msg}")
                self.speak(msg)
                threading.Thread(
                    target=lambda: file_controller({"action": "organize_desktop"}, player=self.ui),
                    daemon=True,
                ).start()
            except Exception as e:
                self.ui.write_log(f"ERR: Desktop organize failed: {e}")
            return True

        # 14. Auto-Tidy Daemon Controls
        if any(k in lower for k in ("auto tidy", "auto-tidy", "autotidy", "auto organize download", "auto organise download")):
            self.ui.write_log(f"You: {text}")
            from actions.auto_tidy_daemon import auto_tidy
            action_type = "start"
            if any(w in lower for w in ("stop", "disable", "turn off", "deactivate", "kill")):
                action_type = "stop"
            elif any(w in lower for w in ("status", "check", "is running", "active")):
                action_type = "status"
            elif any(w in lower for w in ("now", "run now", "immediately", "tidy now")):
                action_type = "run_now"

            res = auto_tidy({"action": action_type}, player=self.ui, speak=self.speak)
            self.ui.write_log(f"Veda AI: {res}")
            self.speak(res)
            return True

        # 15. Window Snapping & Split Screen
        split_match = re.search(r"(?:split screen|split)\s+(?:between\s+)?([a-zA-Z0-9_\s]+?)\s+and\s+([a-zA-Z0-9_\s]+)$", lower)
        snap_both_match = re.search(r"snap\s+([a-zA-Z0-9_\s]+?)\s+(?:to\s+)?(?:the\s+)?(left|right|top|bottom)\s+and\s+([a-zA-Z0-9_\s]+?)\s+(?:to\s+)?(?:the\s+)?(left|right|top|bottom)", lower)
        single_snap_match = re.search(r"snap\s+([a-zA-Z0-9_\s]+?)\s+(?:to\s+)?(?:the\s+)?(left|right|top|bottom|top left|top right|bottom left|bottom right|maximize|center)", lower)

        if split_match:
            self.ui.write_log(f"You: {text}")
            left_app = split_match.group(1).strip()
            right_app = split_match.group(2).strip()
            from actions.window_orchestrator import split_screen
            self.speak(f"Setting up split screen for {left_app} and {right_app}, Rohan.")
            res = split_screen(left_app, right_app)
            self.ui.write_log(f"Veda AI: {res}")
            self.speak(res)
            return True
        ...
        if snap_both_match:
            self.ui.write_log(f"You: {text}")
            app1, pos1, app2, pos2 = snap_both_match.groups()
            from actions.window_orchestrator import snap_window
            self.speak(f"Snapping {app1} {pos1} and {app2} {pos2}, Rohan.")
            snap_window(app1.strip(), pos1.strip())
            time.sleep(0.3)
            snap_window(app2.strip(), pos2.strip())
            msg = f"{app1.strip()} snapped {pos1.strip()}, {app2.strip()} snapped {pos2.strip()}."
            self.ui.write_log(f"Veda AI: {msg}")
            self.speak(msg)
            return True

        if single_snap_match and not any(k in lower for k in ("and", "then")):
            self.ui.write_log(f"You: {text}")
            app = single_snap_match.group(1).strip()
            pos = single_snap_match.group(2).strip().replace(" ", "_")
            from actions.window_orchestrator import snap_window
        ...
        # 16. Smart Form Filler
        if any(lower.startswith(p) for p in ("fill form", "fill the form", "fill this form", "auto fill form", "autofill form", "fill out form", "fill details", "fill in details")):
            self.ui.write_log(f"You: {text}")
            mode = "test_data" if any(w in lower for w in ("test", "synthetic", "mock", "dummy", "fake")) else "profile"
            self.speak(f"Filling the active form with your {mode} data, Rohan.")
            from actions.form_filler import auto_fill_form
            res = auto_fill_form(mode=mode)
            self.ui.write_log(f"Veda AI: {res}")
            self.speak(res)
            return True

        if lower in ("click submit", "submit form", "press submit", "click next", "click enter", "confirm form"):
            self.ui.write_log(f"You: {text}")
            btn = "submit" if "submit" in lower else ("next" if "next" in lower else "enter")
            from actions.form_filler import click_button
            res = click_button(btn)
            self.ui.write_log(f"Veda AI: {res}")
            self.speak(res)
            return True

        # 17. Custom Voice Routines & Macros
        routine_map = {
            "study mode": "study_mode",
            "start study mode": "study_mode",
            "activate study mode": "study_mode",
            "study session": "study_mode",
            "coding session": "coding_session",
            "coding mode": "coding_session",
            "start coding": "coding_session",
            "start coding session": "coding_session",
            "activate coding session": "coding_session",
            "developer mode": "coding_session",
            "deep work": "deep_work",
            "deep work mode": "deep_work",
            "activate deep work": "deep_work",
            "focus mode": "deep_work",
            "meeting setup": "meeting_setup",
            "meeting mode": "meeting_setup",
            "prepare meeting": "meeting_setup",
            "setup meeting": "meeting_setup",
            "relax mode": "relax_mode",
            "break mode": "relax_mode",
            "take a break": "relax_mode",
        }
        for trigger, r_key in routine_map.items():
            if lower == trigger or lower.startswith(trigger + " "):
                self.ui.write_log(f"You: {text}")
                from actions.routines import execute_routine
                res = execute_routine(r_key, player=self.ui, speak=self.speak)
                return True

        if lower in ("list routines", "show routines", "available routines", "what routines", "what are your routines"):
            self.ui.write_log(f"You: {text}")
            from actions.routines import list_routines
            res = list_routines()
            self.ui.write_log(f"Veda AI:\n{res}")
            self.speak("Here are your available productivity routines, Rohan.")
            return True

        # 18. Shutdown / exit
        if lower in ["shutdown veda", "exit veda", "quit veda", "exit app", "quit app", "bye veda"]:
            self.ui.write_log(f"You: {text}")
            self.speak("Goodbye, Rohan. Shutting down Veda.")
            time.sleep(1.2)
            os._exit(0)

        return False

    def _handle_agent_goal(self, text: str, source: str = "local") -> bool:
        lower = text.lower()
        goal_keywords = [
            "presentation", "powerpoint", "slides", "pptx",
            "spreadsheet", "excel", "tracker", "xlsx",
            "word document", "create a docx", "write a document",
            "research report", "deep research",
        ]
        is_compound = any(f" {conj} " in lower for conj in ("and then", "then", "after that")) or (
            any(lower.startswith(p) for p in ("open ", "launch ", "start ", "run ")) and any(k in lower for k in (" and type ", " and write ", " and search ", " and open ", " and close "))
        ) or any(k in lower for k in ("organize my download", "organise my download", "organize download folder", "organise download folder"))

        if not any(k in lower for k in goal_keywords) and not is_compound:
            return False
        ...
        def _run_goal():
            try:
                ...
                self.speak("I am planning and executing this task for you, Rohan.")
                from agent.planner import create_plan
                from agent.executor import AgentExecutor

                plan = create_plan(text)
                steps_desc = [s.get("description", "") for s in plan.get("steps", [])]
                self.ui.begin_task_workspace(text, steps_desc, source=source or "local")

                executor = AgentExecutor(player=self.ui, speak=self.speak)
                result = executor.execute_plan(plan)
                self.ui.finish_task_workspace(str(result)[:300], "Goal completed.", 100)
                self.speak("Goal completed successfully, Rohan.")
            except Exception as e:
                self.ui.write_log(f"ERR: Agent goal failed: {e}")
                self.speak(f"Rohan, I encountered an issue while executing the goal: {e}")

        threading.Thread(target=_run_goal, daemon=True).start()
        return True

    def _listen_audio_seekai_loop(self):
        """Continuous background listener for SeekAI mode using PyAudio/sounddevice and Google Speech Recognition."""
        try:
            ...
            print("[VedaAI] 🎤 SeekAI voice listener online.")

            # Try PyAudio microphone first
            try:
                mic = sr.Microphone()
                with mic as source:
                    recognizer.adjust_for_ambient_noise(source, duration=0.6)
                print("[VedaAI] 🎤 PyAudio microphone calibrated and ready.")
                ...
                while True:
                    ...
                    try:
                        ...
                        if heard_text:
                            print(f"[VedaAI] 🎙️ Heard: {heard_text}")
                            self._on_text_command(heard_text, source="voice")
                ...
                print(f"[VedaAI] ⚠️ PyAudio microphone fallback to sounddevice: {pe}")
                import sounddevice as sd
                import numpy as np
                ...
                while True:
                    ...
                    try:
                        ...
                        heard_text = recognizer.recognize_google(audio_data).strip()
                        if heard_text:
                            print(f"[VedaAI] 🎙️ Heard (sounddevice): {heard_text}")
                            self._on_text_command(heard_text, source="voice")
            ...
            print(f"[VedaAI] Voice listener initialization error: {e}")
    ...
    def _on_text_command(self, text: str, source: str = "local"):
        ...
        # Check direct action handler first
        if self._handle_direct_action(text, source=source):
            return

        # Check autonomous goal handler
        if self._handle_agent_goal(text, source=source):
            return
        ...
        if lower.startswith("email ") or lower.startswith("mail ") or "send email" in lower or "write email" in lower or "compose email" in lower:
            ...
            if not recipient:
                ...
                self.ui.write_log(f"Veda Echo: {prompt}")
                self.speak(prompt)
                try:
        ...
        try:
            ...
            if routed_text_home != text:
                print(f"[VEDA ECHO] Redirection: '{text}' -> '{routed_text_home}'")
                text = routed_text_home
        except Exception as e:
            print(f"[VEDA ECHO] Redirection error: {e}")
        ...
        if website_request:
            if not developer_workspace:
                ...
                developer_workspace = str(default_ws)
            try:
                self.speak("Working on your website, Rohan...")
                if hasattr(self.ui, "_developer_status_lbl"):
                    self.ui._developer_status_lbl.setText(f"Building website in {developer_workspace}")
        ...
        try:
            ...
            pass
        if source != "instagram" and self._handle_veda_connect_command(text, source=source or "local"):
            return
        ...
        if _looks_like_screen_request(text):
            try:
                self.ui.update_task_workspace(
                    ...
                    output="Veda Echo is inspecting the screen for what you asked about.",
                    percent=40,
                )
            except Exception:
                pass
    ...
    def _handle_smart_home_command(self, text: str, source: str = "local") -> bool:
        ...
        connect_words = (
            "phone", "mobile", "android", "tablet", "device", "devices", "veda connect",
        ...
        try:
            ...
            self.ui.write_log(f"Veda Echo: {detail}")
            self.speak(detail)
    ...
    def _handle_veda_connect_command(self, text: str, source: str = "local") -> bool:
        normalized = re.sub(r"\s+", " ", re.sub(r"[^a-z0-9\s%]", " ", text.lower())).strip()
        connect_words = (
            "phone", "mobile", "android", "tablet", "device", "devices", "veda connect",
        ...
        try:
            ...
            if result.get("success", False):
                ...
                title = f"Veda Connect: {action}"
                self.ui.update_task_workspace(
                    title=title,
                    command=text,
                    plan=[
                        "Identify the paired phone or device",
                        "Route the command through Veda Connect",
            ...
            self.ui.write_log(f"ERR: Veda Connect command failed: {result.get('error') or 'Unknown error'}")
            return False
        except Exception:
            return False
    ...
    def _announce_attention(self, event: dict):
        ...
        self.ui.write_log(f"Veda Echo: {msg}")
        if self.session and self._loop:
            self.speak(msg)
        else:
    ...
    def _on_meeting_update(self, payload: dict):
        ...
        if answer:
            self.ui.write_log(f"Veda Echo: {answer}")
    ...
    def _prompt_message_reply(self, event: dict) -> bool:
        ...
        self.ui.write_log(f"Veda Echo: {message}")
        if self.session and self._loop:
            self.speak(message)
        else:
    ...
    def _handle_ig_reply_flow(self, text: str) -> bool:
        ...
        if self._email_step == 0:
            ...
            try:
                import urllib.parse
                import webbrowser
                subject = "Message from Veda Echo"
                ...
                if "gmail" in app_lower or "chrome" in app_lower:
                    import urllib.parse
                    quoted_recipient = urllib.parse.quote(self._email_recipient)
                    quoted_subject = urllib.parse.quote("Message from Veda Echo")
                    quoted_body = urllib.parse.quote(self._email_message)
    ...
    def _handle_attention_response(self, text: str) -> bool:
        ...
        if kind == "message":
            ...
            if self._attention_matches(lower, ("hear", "read", "what is it", "tell me", "show it", "open it")):
                ...
                self.ui.write_log(f"Veda Echo: {preview}")
    ...
    def _fallback_reply(self, text: str, memory_ctx: str = ""):
        try:
            self.ui.set_state("THINKING")
            clean_text = _clean_command_text(text)

            # Check if cleaned text can be handled directly
            if self._handle_direct_action(clean_text):
                return

            # Check if an autonomous tool action should be planned and executed
            lower_clean = clean_text.lower()
            is_action_likely = any(
                lower_clean.startswith(act) for act in (
                    "open ", "launch ", "start ", "run ", "play ", "search ", "find ", "look up ",
                    "create ", "make ", "build ", "generate ", "write ", "download ", "install ",
                    "update ", "close ", "kill ", "stop ", "mute ", "unmute ", "turn ", "set ",
                    "volume ", "organize ", "clean ", "screenshot ", "remind ", "send ", "email ",
                    "schedule ", "take a ", "show me ", "type ", "press ", "hotkey ", "click ",
                    "scroll ", "copy ", "paste ", "focus ", "switch to ", "snap ", "split ", "fill "
                )
            ) or any(act in lower_clean for act in (
                "presentation", "powerpoint", "slides", "pptx", "spreadsheet", "excel", "xlsx",
                "document", "docx", "desktop", "battery", "weather", "spotify", "youtube",
                "notepad", "download", "folder", "type ", "write in ", "write on ",
                "auto tidy", "split screen", "routine", "study mode", "coding session", "fill form"
            ))
            ...
            if is_action_likely:
                try:
                    ...
                    self.ui.update_task_workspace(
                        title=f"Planning: {clean_text[:40]}",
                        status="Planning",
                        output="Veda AI is analyzing your request to execute...",
                        percent=25,
                    )
                    plan = create_plan(clean_text, context=memory_ctx)
                    steps = plan.get("steps", [])
                    if steps and not (len(steps) == 1 and steps[0].get("tool") == "web_search" and not any(k in lower_clean for k in ("search", "find", "google", "look up"))):
                        steps_desc = [s.get("description", "") for s in steps]
                        self.ui.begin_task_workspace(clean_text, steps_desc)
                        first_step = steps[0].get("description") or f"Executing {steps[0].get('tool')}"
                        self.speak(f"On it, Rohan. {first_step}")

                        executor = AgentExecutor(player=self.ui, speak=self.speak)
                        result = executor.execute_plan(plan)

                        summary = str(result).strip() if result else "Task completed successfully."
                        self.ui.write_log(f"Veda AI: {summary[:300]}")
                        self.speak(summary[:300])
                        self.ui.finish_task_workspace(summary, "Task completed.", 100)
                        if not self.ui.muted:
                            self.ui.set_state("LISTENING")
                        return
                except Exception as pe:
                    print(f"[VedaAI] Autonomous plan execution exception: {pe}")

            try:
                self.ui.update_task_workspace(
                    status="Thinking",
                    output="Veda AI is drafting a direct reply.",
                    percent=35,
                )
            except Exception:
                pass
            ...
            system_instruction = (
                "You are Veda AI (GPT-6 Astra Edition), an ultra-capable autonomous AI assistant for Rohan. "
                "You operate directly on Rohan's Windows computer with deep system integration, browser control, software tools, and 11 student academic plugins. "
                "Reply naturally, concisely, and directly. "
                "Do not pretend to have performed system actions or typed into apps unless actual tool execution occurred."
            )
            ...
            if gemini_first:
                try:
                    ...
                    print(f"[VedaAI] ⚠️ Gemini fallback failed: {e}")
            ...
            if not reply:
                try:
                    ...
                    print(f"[VedaAI] ⚠️ SeekAI / OpenRouter fallback failed: {e}")
                    if gemini_first and not self._use_openrouter_first and _is_gemini_limit_error(e):
                        self._use_openrouter_first = True
            reply = (reply or "").strip()
            if not reply:
                reply = "I’m ready, Rohan."
            self.ui.write_log(f"Veda AI: {reply}")
            self.speak(reply)
            try:
        ...
        if self.session and self._loop:
            ...
            async def _send():
                try:
                    ...
                    print(f"[VEDA ECHO] Unified Speak err: {e}")
            asyncio.run_coroutine_threadsafe(_send(), self._loop)
        else:
    ...
    def _build_config(self) -> types.LiveConnectConfig:
        ...
        parts.append(
            "Wake-word mode: if the microphone is muted, still listen for the words 'Veda Echo', 'hey', 'hi', and 'hello'. "
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        try:
            if name == "computer_settings":
                ...
                r = await loop.run_in_executor(None, lambda: browser_control(parameters=args, player=self.ui, speak=self.speak))
                result = r or "Done."
        ...
        if tool_voice:
            try:
                self.ui.write_log(f"Veda Echo: {tool_voice}")
            except Exception:
                pass
            try:
        ...
        print(f"[VEDA ECHO] 📤 {name} → {str(result)[:80]}")
    ...
    async def _listen_audio(self):
        print("[VEDA ECHO] 🎤 Mic started")
        loop = asyncio.get_event_loop()
        import numpy as np

        def callback(indata, frames, time_info, status):
            with self._speaking_lock:
                veda_speaking = self._is_speaking
            ...
            if not self.ui.muted or getattr(self.ui, "_wakeword_listening", False):
                ...
                threshold = 1200.0 if veda_speaking else 10.0
        ...
        try:
            with sd.InputStream(
                ...
                print("[VEDA ECHO] 🎤 Mic stream open")
                while True:
                    await asyncio.sleep(0.1)
        except Exception as e:
            print(f"[VEDA ECHO] ❌ Mic: {e}")
            raise

    async def _receive_audio(self):
        print("[VEDA ECHO] 👂 Recv started")
        out_buf, in_buf = [], []
        ...
        try:
                ...
                async for response in self.session.receive():
                    ...
                    if response.server_content:
                        ...
                        if sc.turn_complete:
                            ...
                            if full_out:
                                self.ui.write_log(f"Veda Echo: {full_out}")
                            out_buf = []
            ...
            print(f"[VEDA ECHO] ❌ Recv: {e}")
            traceback.print_exc()
            raise

    async def _play_audio(self):
        print("[VEDA ECHO] 🔊 Play started")
        loop = asyncio.get_event_loop()
    ...
    async def run(self):
        ...
        is_genuine_google_key = bool(gemini_key and gemini_key.startswith("AIza"))

        if not is_genuine_google_key:
            print("[VedaAI] Non-Google key detected. Engaging SeekAI Primary Mode.")
            self._use_openrouter_first = True
            self._loop = asyncio.get_event_loop()
            try:
                self.ui.boot_set_step_status("Connect AI backend", "done")
                self.ui.boot_set_progress(75, "SeekAI backend ready")
                ...
                pass
            self.ui.set_state("LISTENING")
            self.ui.write_log("SYS: Veda AI - Lite online (SeekAI Mode).")

            threading.Thread(target=self._listen_audio_seekai_loop, daemon=True, name="seekai-mic-listener").start()
        ...
        while True:
                ...
                try:
                    async with asyncio.TaskGroup() as tg:
                        ...
                        try:
                            ...
                            pass
                        self.ui.set_state("LISTENING")
                        self.ui.write_log("SYS: Veda Echo online.")
            ...
            self.set_speaking(False)
            self.ui.set_state("LISTENING")
            print("[VEDA ECHO] 🔄 Reconnecting in 5s...")
...
def main():
    ...
    ui = VedaUI(str(BASE_DIR / "assets" / "Veda_Lite_Logo.png"), show_immediately=True)
    dashboard = None
    ...
    if DashboardServer is not None and not dashboard_enabled:
        ...
        try:
            ui.write_log("SYS: Mobile Connect is already running in another Veda Echo instance.")
        except Exception:
            pass
    ...
    veda_connect = None
    veda_connect_enabled = False
    if get_veda_connect_service is not None:
        try:
            veda_connect = get_veda_connect_service(BASE_DIR)
            veda_connect_enabled = bool(veda_connect.gateway.config.enabled)
        except Exception as exc:
            _startup_log(f"veda connect init failed: {exc}")
            try:
                ui.write_log(f"ERR: Veda Connect failed to initialize: {exc}")
            except Exception:
                pass
            veda_connect = None
    try:
        if veda_connect is not None and hasattr(ui, "set_veda_connect_service"):
            ui.set_veda_connect_service(veda_connect)
    except Exception:
        pass
    if veda_connect is not None and veda_connect_enabled:
        connect_port = int(getattr(veda_connect.gateway.config, "port", 8765))
        if _is_port_in_use(connect_port):
            _startup_log(f"veda connect disabled: port {connect_port} already in use")
            try:
                ui.write_log(f"SYS: Veda Connect is already running on port {connect_port}.")
            except Exception:
                pass
        else:
            def _start_veda_connect_server():
                try:
                    _startup_log("veda connect thread started")
                    veda_connect.start_background()
                    _startup_log("veda connect thread spawned")
                except Exception as exc:
                    _startup_log(f"veda connect thread error: {exc}")
                    try:
                        ui.write_log(f"ERR: Veda Connect server failed: {exc}")
                    except Exception:
                        pass

            threading.Thread(target=_start_veda_connect_server, daemon=True).start()
    ...
    def runner():
        ...
        try:
            if plugin_manager is not None:
                veda_echo.plugin_manager = plugin_manager
                plugin_manager.register_veda(veda_echo)
                if dashboard is not None and hasattr(dashboard, "set_plugin_manager"):
                    try:
                        dashboard.set_plugin_manager(plugin_manager)
                    except Exception:
                        pass
                # allow plugins to run a startup hook
                try:
                    plugin_manager.dispatch("on_startup", veda_echo)
                except Exception:
                    pass
        except Exception:
            pass
        ...
        if start_ig_daemon:
            ...
            def _ig_handler(thread_id, username, text, is_auto):
                if is_auto:
                    ...
                    veda_echo._ig_reply_mode = True
                    veda_echo._ig_pending_thread = {
                        "thread_id": thread_id,
                        "username": username,
                        "message": text
                    }
                ...
                try:
                    ...
                    if curr_clip != last_clip:
                        ...
                        if text and len(text) > 3:
                            ...
                            ui.write_log(f"Veda Echo (Clipboard): {reply}")
                            veda_echo.speak(reply)
                except Exception:
                    pass
