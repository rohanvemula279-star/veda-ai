"""
dashboard/server.py — Veda Local HTTP Dashboard
...
def _make_uploads_dir() -> Path:
    ...
    for candidate in [
        Path.home() / "Downloads" / "Veda Uploads",
        Path.home() / "Documents" / "Veda Uploads",
        BASE_DIR / "uploads",
    ]:
        try:
...
def _ensure_network_access(port: int) -> None:
    ...
    if sys.platform == "win32":
        ...
        port_rule = f"Veda Dashboard Port {port}"
        prog_rule  = "Veda Dashboard Python"
        ...
        bat_body = "\r\n".join(bat_lines) + "\r\n"
        fd, bat_path = tempfile.mkstemp(suffix=".bat", prefix="veda_fw_")
        try:
        ...
        try:
            ...
            if int(ret) > 32:
                ...
                print("[Dashboard] Phone connections may fail until Veda is run as Administrator.")
...
def _ensure_ssl_certs() -> bool:
    ...
    key_path = certs / "veda.key"
    cert_path = certs / "veda.crt"
    ...
    try:
        ...
        subject = issuer = x509.Name([
            x509.NameAttribute(NameOID.COMMON_NAME, "Veda AI Local Remote"),
        ])
        alt_names = [
...
class DashboardServer:
    ...
    def _build_app(self) -> "FastAPI":
        ...
        async def local_session(req: Request):
            client_ip = req.client.host if req.client else ""
            if client_ip in ("127.0.0.1", "::1", "localhost", self._ip):
                tok = secrets.token_urlsafe(32)
                self._tokens.add(tok)
                self._token_keys[tok] = "LOCAL"
                return JSONResponse({"ok": True, "token": tok, "key": "LOCAL"})
            return JSONResponse({"ok": False, "error": "Not local"}, status_code=403)
...
<p>Press <strong style="color:#dde3ed">Mobile Connect</strong> in Veda to get a new QR code.</p>
</div></body></html>""")
...
<body>
<script>
  sessionStorage.setItem('veda_token','{tok}');
  sessionStorage.setItem('veda_key','{key}');
  localStorage.setItem('veda_device_token','{dev_tok}');
  setTimeout(function(){{location.replace('/')}},400);
</script>
<p>Connecting to Veda…</p>
</body></html>""")
        ...
        async def list_plugins(req: Request):
            ...
            if pm is None:
                ...
                pm = PluginManager(BASE_DIR)
                pm.load_plugins()
            return JSONResponse({"ok": True, "plugins": pm.get_plugins_metadata()})
        ...
        async def toggle_plugin_endpoint(req: Request):
            ...
            if pm is None:
                ...
                pm = PluginManager(BASE_DIR)
                pm.load_plugins()
            success = pm.toggle_plugin(pid, enabled)
            return JSONResponse({"ok": success, "id": pid, "enabled": enabled})
        ...
        async def system_status_ep(req: Request):
            ...
            try:
                ...
                if batt:
                    batt_info = {"percent": batt.percent, "plugged": batt.power_plugged}
                cpu_val = psutil.cpu_percent(interval=0.02)
                ram_val = psutil.virtual_memory().percent
            except Exception:
                pass
            p_count = len(self._plugin_manager.plugins) if self._plugin_manager else 11
            return JSONResponse({
                "ok": True,
                "cpu": cpu_val,
                "ram": ram_val,
                "battery": batt_info,
                "plugins_count": p_count,
            })
    ...
    async def _serve_alias(self) -> None:
        ...
        ssl_key  = BASE_DIR / "config" / "certs" / "veda.key"
        ssl_cert = BASE_DIR / "config" / "certs" / "veda.crt"
    ...
    async def serve(self) -> None:
        ...
        print("[Dashboard] Press 'Mobile Connect' in Veda UI to get the QR code.")
        await uvicorn.Server(cfg).serve()
